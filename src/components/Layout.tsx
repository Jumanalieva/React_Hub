    ...
    <div className="flex flex-col min-h-screen bg-stone-200 text-stone-700 font-custom">
      <header className="bg-stone-200 shadow-md w-full fixed top-0 z-50">
        ...
        {isOpen && (
          <div className="fixed inset-0 flex flex-col items-center justify-between bg-stone-200 z-50 md:hidden">
            <div className="absolute top-4 left-4">
              <h1 className="text-black text-2xl">SENIOR HUB</h1>
            </div>
            <button
            ...
            <div className="flex flex-col items-center mt-24 space-y-3">
              <Link to="/" onClick={handleLinkClick} className={`block py-3 text-5xl ${location.pathname === '/' ? 'underline' : ''}`}>HOME</Link>
              <Link to="/mission" onClick={handleLinkClick} className={`block py-4 text-5xl ${location.pathname === '/mission' ? 'underline' : ''}`}>MISSION</Link>
              <Link to="/aboutus" onClick={handleLinkClick} className={`block py-4 text-5xl ${location.pathname === '/aboutus' ? 'underline' : ''}`}>ABOUT US</Link>
              <Link to="/activities" onClick={handleLinkClick} className={`block py-4 text-5xl ${location.pathname === '/activities' ? 'underline' : ''}`}>ACTIVITIES</Link>
              <Link to="/events" onClick={handleLinkClick} className={`block py-4 text-5xl ${location.pathname === '/events' ? 'underline' : ''}`}>EVENTS</Link>
              {!isAuthenticated ? (
                <>
                  <Link to="/" onClick={authOnClick('signup')} className={`block py-4 text-5xl text-red-700 ${location.pathname === '/signup' ? 'underline' : ''}`}>SIGN UP</Link>
                  <Link to="/" onClick={authOnClick('login')} className={`block py-4 text-5xl text-red-700 ${location.pathname === '/login' ? 'underline' : ''}`}>LOG IN</Link>
                </>
              ) : (
                <Link to="/" onClick={signOutOnClick} className="block py-4 text-5xl text-red-700">SIGN OUT</Link>
              )}
            </div>
        ...
        <nav className="hidden md:flex md:flex-row md:space-x-8 md:w-full md:justify-center py-4 text-black">
          ...
          <Link to="/mission" className={`text-xl lg:text-2xl xl:text-2xl hover:underline ${location.pathname === '/mission' ? 'underline' : ''}`}>MISSION</Link>
          <Link to="/aboutus" className={`text-xl lg:text-2xl xl:text-2xl hover:underline ${location.pathname === '/aboutus' ? 'underline' : ''}`}>ABOUT US</Link>
          <Link to="/activities" className={`text-xl lg:text-2xl xl:text-2xl hover:underline ${location.pathname === '/activities' ? 'underline' : ''}`}>ACTIVITIES</Link>
          <Link to="/events" className={`text-xl lg:text-2xl xl:text-2xl hover:underline ${location.pathname === '/events' ? 'underline' : ''}`}>EVENTS</Link>
          {!isAuthenticated ? (
            <>
